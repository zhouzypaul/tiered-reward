...
from reward.minigrid.dqn_agent import make_dqn_agent
from reward.minigrid.ppo_agent import make_ppo_agent
from reward.minigrid.minigrid_wrappers import environment_builder, make_batch_env
from reward.minigrid import utils as minigrid_utils
...
def batch_run_eval_episodes(
    ...
    with agent.eval_mode():
        ...
        obss = env.reset()
        # obss = [obs[0] for obs in obss]  # get rid of the info dict for mingrid 
        ...
        while True:
            ...
            timestep += 1
            # o_{t+1}, r_{t+1}
            obss, rs, truncated, dones, infos = env.step(actions)
...
def train_agent_batch_with_evaluation(
    ...
    # o_0, r_0
    obss = env.reset()
    # obss = [obs[0] for obs in obss]  # get rid of the info dict for minigrid
...
def main():
    ...
    # agent
    sample_env = environment_builder(args.env, seed=args.seed, use_img_obs=True, reward_fn='original', grayscale=False, max_steps=None)  # TOOD: change
    agent = make_ppo_agent(
        sample_env.observation_space.shape, sample_env.action_space.n, lambda x: x.astype(np.float32),
    )
    # agent = make_dqn_agent(args, n_actions=sample_env.action_space.n, gamma=args.gamma, preprocess_func=switch_channel)
    ...
    process_seeds = [int(s) for s in process_seeds]  # np.int64 -> int for gymnasium
