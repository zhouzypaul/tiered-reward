...
class CrossingMiniGridTierReward(TierRewardWrapper):
    ...
    def populate_bfs_distance(self):
        ...
        while len(unvisited_locs)>0:
            ...
            if isinstance(self.env.grid.get(unvisited_locs[1],unvisited_locs[0])) is Wall:
                bfs_matrix[curr_loc[1], curr_loc[0]] = -1
                continue
            elif isinstance(self.env.grid.get(unvisited_locs[1],unvisited_locs[0])) is Lava:
                bfs_matrix[curr_loc[1], curr_loc[0]] = -1
                continue
    ...
    def _get_tier(self, info):
        ...
        if bfs_matrix[player_pos[1], player_pos[0]] < 0:
            ...
            out = 1 + math.round((self.num_tiers-2)*(1 - (bfs_matrix[player_pos[1], player_pos[0]]/np.max(bfs_matrix)) ))
